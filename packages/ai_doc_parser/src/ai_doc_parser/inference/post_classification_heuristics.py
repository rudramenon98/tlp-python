...
def _apply_heuristics(self, elements: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for i in range(len(corrected_elements)):
        ...
        if i == len(corrected_elements) - 1:
            if (
                i > 0
                and corrected_elements[i - 1]["type"] not in [self.HEADING, 3]
                and current_type == 3
            ):
                corrected_elements[i]["type"] = self.HEADING
            continue
        ...
        if (
            prev_type == self.TOC
            and next_type == self.TOC
            and current_type != self.HEADING
        ):
            corrected_elements[i]["type"] = self.TOC
...
def post_classification_heuristics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    corrected_df.loc[starts_and_ends_with_digit(corrected_df), "FinalClass"] = (
        TextClass.TOC
    )
    ...
    corrected_df.loc[
        ...
        & (
            corrected_df["font_size_binned"]
            != corrected_df["font_size_binned"].shift(1)
        ),
        "FinalClass",
    ] = TextClass.HEADING
    ...
    corrected_df.loc[
        ...
        & (corrected_df["FinalClass"] != TextClass.HEADING)
        & (next_line == TextClass.HEADING_CONT),
        "FinalClass",
    ...
    corrected_df.loc[
        is_first_line & (corrected_df["FinalClass"] == TextClass.HEADING_CONT),
        "FinalClass",
    ] = TextClass.HEADING
    ...
    bullet_mask = bullet_mask | corrected_df["text"].str.strip().str.startswith(
        "[#x2022;]"
    )
    ...
    corrected_df.loc[
        (corrected_df["FinalClass"] == TextClass.BULLET_LIST)
        & ~corrected_df["text"].str.match(bullet_regex),
        "FinalClass",
    ] = TextClass.PARAGRAPH
    ...
    for textclass, textclass_cont in CONTINUE_PAIRS:
        ...
        mask = (
            ...
            & (
                prev_line_class != textclass_cont
            )  # Previous line is not the continue class
        )
    ...
    corrected_df = pd.concat(
        [corrected_df, df[~df["PredictedClass"].isin(AI_PARSED_CLASSES)]]
    )
