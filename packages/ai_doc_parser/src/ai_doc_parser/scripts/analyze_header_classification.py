...
def analyze_header_classification(model_path: str, features_path: str, row_index: int):
    ...
    all_header_features = sorted(
        all_header_features,
        key=lambda x: feature_importance[feature_columns.index(x)],
        reverse=True,
    )
    ...
    print(
        f"{'Row':<5} {'Predicted':<15} {'True':<15} {'Confidence':<12} {'Key Features':<30}"
    )
    print(f"{'-'*80}")
    ...
    for i in range(start_idx, end_idx):
        if i < len(X):
            ...
            print(
                f"{i:<5} {pred_name:<15} {true_name:<15} {max(context_proba):<12.3f} {key_features_str:<30}{marker}"
            )
    ...
    try:
        ...
        explanation.save_to_file(
            f"header_analysis_output/lime_explanation_row_{row_index}.html"
        )
        print(
