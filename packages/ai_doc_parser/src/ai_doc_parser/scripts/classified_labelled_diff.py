...
def get_labelled_classified_diff(classified_df: pd.DataFrame, labelled_df: pd.DataFrame) -> pd.DataFrame:
    # Merge the dataframes on pdf_idx to get matching rows efficiently
    merged_df = classified_df.merge(labelled_df, on='pdf_idx', how='left', suffixes=('', '_labelled'))
...
def main() -> None:
    ...
    for pdf_path in pdf_paths:
        ...
        output_path = pdf_path.parent / "classified_labelled_diff" / f"{pdf_path.stem}.csv"
