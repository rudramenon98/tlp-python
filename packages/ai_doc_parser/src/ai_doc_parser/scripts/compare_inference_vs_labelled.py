...
def main():
    ...
    output_path = (
        labelled_path.parent.parent / "comparison" / f"{labelled_path.stem}.csv"
    )
    ...
    for i, row in labelled_df.iterrows():
        ...
        if inference_row["FinalClass"] != row["LabelledClass"]:
            ...
            comparison_row["PredictedClass"] = TextClass(
                inference_row["PredictedClass"]
            )
            comparison_row["LabelledClassName"] = TextClass(row["LabelledClass"]).name
            comparison_row["PredictedClassName"] = TextClass(
                inference_row["PredictedClass"]
            ).name
            comparison_row["FinalClass"] = TextClass(inference_row["FinalClass"])
            comparison_row["FinalClassName"] = TextClass(
                inference_row["FinalClass"]
            ).name
