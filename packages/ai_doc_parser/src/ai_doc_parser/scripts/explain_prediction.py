...
def explain_prediction(
    model_path: str, data_path: str, row_index: int, num_features: int = 10
):
    """
    ...
    for _, row in top_features.iterrows():
        ...
        print(
            f"{row['feature']:<35} {row['value']:<10.3f} {row['importance']:<12.4f} {contribution:<12.4f}"
        )
    ...
    try:
        ...
        explanation = lime_explainer.explain_instance(
            sample.values[0], model.predict_proba, num_features=num_features
        )
...
def compare_predictions(
    model_path: str, data_path: str, row_indices: list, num_features: int = 10
):
    """
    ...
    if len(explanations) > 1:
        ...
        for feature in sorted(all_features):
            print(f"{feature:<35}", end="")
            for exp in explanations:
                feature_data = exp["top_features"][
                    exp["top_features"]["feature"] == feature
                ]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Explain why a specific row was classified the way it was"
    )
    parser.add_argument(
    ...
    if args.row_index is not None:
        explain_prediction(
            args.model_path, args.data_path, args.row_index, args.num_features
        )
    elif args.row_indices is not None:
        row_indices = [int(x.strip()) for x in args.row_indices.split(",")]
        compare_predictions(
            args.model_path, args.data_path, row_indices, args.num_features
        )
    else:
