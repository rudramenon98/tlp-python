...
def Canada_Guidance_Scraping(url, docker_url):
    ...
    all_list_elements = driver.find_elements(
        By.XPATH, "/html/body/div/div/main/div[1]/header"
    )
    dataset = {
        "Number": [],
        "title": [],
...
def check_for_cancelled_documents(
    ...
    old_documents_list = find_documents_not_scraped_on_date_by_type(
        mysql_driver, current_date, document_type=scrapeScript.documentTypeID
    )
...
def run(config: ScriptsConfig, scrapeURLId):
    ...
    retval = check_for_new_documents(
        config, mysql_driver, scrapeDF, scrapeURLId, scrapeScript=scrapeScript
    )

    # handle cancelled documents
    cancel_list = check_for_cancelled_documents(
        mysql_driver, DateToday, scrapeURLId, scrapeScript=scrapeScript
    )
