...
@Error_Handler
def scrape_510K_records(
    config: ScriptsConfig, fda_510k_text_files, mysql_driver, scrapeURLId
):
    # dfs = []
    ...
    try:
        ...
        for row in fda_510k_text_files:
            ...
            dataset["active_date"].append(
                datetime.strptime(row["active_date"], "%m/%d/%Y").date()
            )
            dataset["Number"].append(row["Number"])
            dataset["applicant"].append(row["applicant"])
            dataset["contact"].append(row["contact"])
            dataset["receivedDate"].append(
                datetime.strptime(row["receivedDate"], "%m/%d/%Y").date()
            )
            dataset["decisionDate"].append(
                datetime.strptime(row["decisionDate"], "%m/%d/%Y").date()
            )
            ...
            dataset["classificationAdvisoryCommittee"].append(
                row["classificationAdvisoryCommittee"]
            )
...
def find_510K_record_by_k_number(mysql_driver, number):
    ...
    result = session.query(K510Record).filter(K510Record.k_number == number).first()
    session.close()
    return result


def insert_510K_records(
        ...
        try:
            if len(k_number) == 0:
                ...
                logText = (
                    "Skipped File "
                    + row["title"]
                    + " with Premarket Notificaiton at "
                    + row["Number"]
                    + " URL = <null>  on "
                    + str(datetime.today())
                )
        ...
        if not docInDB:
            try:
                record = K510Record(
                    ...
                    classificationAdvisoryCommittee=row[
                        "classificationAdvisoryCommittee"
                    ],
                    Type=row["type"],
                ...
                logText = (
                    "510(k) Record "
                    + row["title"]
                    + " with FDA 510(k) Premarket Notifications scraped on "
                    + str(datetime.today())
                )
                logList.append(logText)
                scrape_url_append_log(mysql_driver, scrapeURLId, logText)
            ...
            logText = (
                "510(K) Record "
                + row["title"]
                + " with FDA 510(k) Premarket Notifications scraped and not changed on "
                + str(datetime.today())
            )
            logList.append(logText)
            scrape_url_append_log(mysql_driver, scrapeURLId, logText)
...
def run(config: ScriptsConfig, scrapeURLId):
    ...
    if firstTime:
        ...
        scrapeDF = scrape_510K_records(
            config, fda_510k_text_files, mysql_driver, scrapeURLId
        )

    insert_510K_records(config, mysql_driver, scrapeDF, scrapeURLId, scrapeScript)
